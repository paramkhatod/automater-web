...
const TextType = ({
  text,
  texts, // Added 'texts' support for consistency
  as: Component = 'div',
  typingSpeed = 50,
  initialDelay = 0,
  pauseDuration = 2000,
  ...
  // ARCHITECT FIX: Support both 'text' and 'texts' props safely
  ...
  useEffect(() => {
    if (showCursor && cursorRef.current) {
      ...
      return () => anim.kill(); // Memory safety
    }
  ...
  useEffect(() => {
    ...
    let timeout;
    const currentText = textArray[currentTextIndex] || ""; // Safety guard
    ...
    timeout = setTimeout(executeTypingAnimation, (currentCharIndex === 0 && !isDeleting && displayedText === '') ? initialDelay : 0);
  ...
  // ARCHITECT FIX: Added Optional Chaining (?.) to prevent the "length" crash
  const currentItem = textArray[currentTextIndex];
  const shouldHideCursor = hideCursorWhileTyping && (currentCharIndex < (currentItem?.length || 0) || isDeleting);
